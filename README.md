...
**修复：她自己的语音把我们的话"掐一半"，剩下的半句又冒出来**

- 游戏每次自己开口（剧情台词、野生动作的碎碎念……）都会先把它那条语音通道整个停掉 —— 我们借它播的语音也会被一起掐掉。以前我们只停自己的播放器，结果音频断了、字幕还挂在屏幕上、连播的第二句还照念，听感就是"话说一半断了，等她的动作完了又接上后半段"
- 现在**游戏要开口的瞬间我们就主动收干净**：音频、字幕、口型、连播里剩下的句子一起作废，不再和她抢话

**性能**

- 扫窗口从每 0.2 秒放宽到 **0.5 秒**；最贵的 DWM 查询挪到便宜过滤之后；进程路径加 10 秒缓存
- 场景扫描（`FindObjectsOfTypeAll`）从 1 秒一次放宽到 **5 秒一次**
- 语音解码缓存 40 → **64 条**，减少反复"解包 → 解码"带来的卡顿
- 新增**慢操作探针**：mod 里任何一步超过 30ms 就会在 BepInEx 日志里写一行 `慢操作：xxx`，方便定位掉帧到底是谁造成的

**安装**

- 发布文件统一叫 `ChillClock.dll`，**直接覆盖旧文件**即可（不要把两个不同名字的副本同时放进 `plugins/`，BepInEx 会把同一套功能加载两遍）
