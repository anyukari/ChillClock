...
- **1172 条**台词，覆盖专注闲聊、休息提醒、走神提醒、任务管理器、退出拦截、点击回应六类场景
- 大量**联动台词**：不少场景是"一次说两三句"的连播，并且按时段（早 / 午 / 晚 / 夜）分开，不会串场
- 语音包**内嵌在 DLL 里**，发布形态只有一个 `ChillClock.dll`（约 57 MB），不需要额外的语音文件夹

**点击聪音**

- 专注中点她，她会**一边干活一边回头**回应（用游戏自己的 look 参数 0 / 0.5 / 1），不会停下手里的活
- 她还在说话时点她，走**游戏自己的"现在不能反应"**（禁止光标），不打断、不叠音
- 她自己要开口时，我们先把我们那句收干净，不和她抢话
